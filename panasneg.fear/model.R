...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(panasneg.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(panasneg.fear))) {
    ...
    std_coefs <- coef(beta(panasneg.fear))
  ...
  if(!(is.null(panasneg.fear_sw))){
    coefs_sw <- panasneg.fear_sw
