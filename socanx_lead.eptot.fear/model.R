...
processVoxel <- function(v) {
  ...
  model_formula <- LSA_ANX_lead ~ 1 + TIMECENTER + WCEN_EPISODICTOT + GCEN_EPISODICTOT + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(socanx_lead.eptot.fear <-
              nlme::lme(LSA_ANX_lead ~ 1 + TIMECENTER + WCEN_EPISODICTOT + GCEN_EPISODICTOT + GCEN_BRAIN + WCEN_BRAIN,
  ...
  if (!(is.null(socanx_lead.eptot.fear))) {
    ...
    std_coefs <- coef(beta(socanx_lead.eptot.fear))
  ...
  if(!(is.null(socanx_lead.eptot.fear_sw))){
    coefs_sw <- socanx_lead.eptot.fear_sw
