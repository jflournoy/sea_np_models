...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(ip_chsev.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(ip_chsev.fear))) {
    ...
    std_coefs <- coef(beta(ip_chsev.fear))
  ...
  if(!(is.null(ip_chsev.fear_sw))){
    coefs_sw <- ip_chsev.fear_sw
