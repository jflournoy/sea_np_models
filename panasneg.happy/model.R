...
processVoxel <- function(v) {
  ...
  model_formula <- PANASNEG ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(panasneg.happy <-
              nlme::lme(PANASNEG ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  e_sandwich <- try(panasneg.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(panasneg.happy))) {
    ...
    std_coefs <- coef(beta(panasneg.happy))
  ...
  if(!(is.null(panasneg.happy_sw))){
    coefs_sw <- panasneg.happy_sw
