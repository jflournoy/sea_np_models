...
processVoxel <- function(v) {
  ...
  model_formula <- GAD7_TOT_lead ~ 1 + TIMECENTER + WCEN_EPISODICTOT + GCEN_EPISODICTOT + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(gad_lead.eptot.fear <-
              nlme::lme(GAD7_TOT_lead ~ 1 + TIMECENTER + WCEN_EPISODICTOT + GCEN_EPISODICTOT + GCEN_BRAIN + WCEN_BRAIN,
  ...
  if (!(is.null(gad_lead.eptot.fear))) {
    ...
    std_coefs <- coef(beta(gad_lead.eptot.fear))
  ...
  if(!(is.null(gad_lead.eptot.fear_sw))){
    coefs_sw <- gad_lead.eptot.fear_sw
