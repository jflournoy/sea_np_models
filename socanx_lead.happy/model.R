...
processVoxel <- function(v) {
  ...
  model_formula <- LSA_ANX_lead ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(socanx_lead.happy <-
              nlme::lme(LSA_ANX_lead ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  if (!(is.null(socanx_lead.happy))) {
    ...
    std_coefs <- coef(beta(socanx_lead.happy))
  ...
  if(!(is.null(socanx_lead.happy_sw))){
    coefs_sw <- socanx_lead.happy_sw
