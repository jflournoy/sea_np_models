...
processVoxel <- function(v) {
  ...
  model_formula <- LSA_ANX_lead ~ 1 + TIMECENTER + WCEN_CHRONICSEV + GCEN_CHRONICSEV + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(socanx_lead.chsev.fear <-
              nlme::lme(LSA_ANX_lead ~ 1 + TIMECENTER + WCEN_CHRONICSEV + GCEN_CHRONICSEV + GCEN_BRAIN + WCEN_BRAIN,
  ...
  if (!(is.null(socanx_lead.chsev.fear))) {
    ...
    std_coefs <- coef(beta(socanx_lead.chsev.fear))
  ...
  if(!(is.null(socanx_lead.chsev.fear_sw))){
    coefs_sw <- socanx_lead.chsev.fear_sw
