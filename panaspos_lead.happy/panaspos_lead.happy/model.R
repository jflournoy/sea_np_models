...
processVoxel <- function(v) {
  ...
  model_data <- data.frame(
    PANASPOS_lead = PANASPOS_lead, 
    TIMECENTER = TIMECENTER, 
    GCEN_BRAIN = GCEN_BRAIN, 
    WCEN_BRAIN = WCEN_BRAIN, 
    idnum = idnum)

  model_formula <- PANASPOS_lead ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(panaspos_lead.happy <-
              nlme::lme(PANASPOS_lead ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  if (!(is.null(panaspos_lead.happy))) {
    ...
    std_coefs <- coef(beta(panaspos_lead.happy))
  ...
  if(!(is.null(panaspos_lead.happy_sw))){
    coefs_sw <- panaspos_lead.happy_sw
