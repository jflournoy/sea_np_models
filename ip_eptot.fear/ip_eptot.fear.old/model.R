...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(ip_eptot.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(ip_eptot.fear))) {
    ...
    std_coefs <- coef(beta(ip_eptot.fear))
  ...
  if(!(is.null(ip_eptot.fear_sw))){
    coefs_sw <- ip_eptot.fear_sw
