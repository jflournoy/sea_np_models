...
processVoxel <- function(v) {
  ...
  model_formula <- PHQ9_TOT ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(dep.fear <-
              nlme::lme(PHQ9_TOT ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  if (!(is.null(dep.fear))) {
    ...
    reg_resids <- rep(NOVAL, dim(model_data)[1]) 
  }
