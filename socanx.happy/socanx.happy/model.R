...
processVoxel <- function(v) {
  ...
  model_formula <- LSA_ANX ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(socanx.happy <-
              nlme::lme(LSA_ANX ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  e_sandwich <- try(socanx.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(socanx.happy))) {
    ...
    std_coefs <- coef(beta(socanx.happy))
  ...
  if(!(is.null(socanx.happy_sw))){
    coefs_sw <- socanx.happy_sw
