...
processVoxel <- function(v) {
  ...
  model_formula <- internalizing ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(int_lead.fear <-
              nlme::lme(internalizing ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  e_sandwich <- try(int_lead.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(int_lead.fear))) {
    ...
    std_coefs <- coef(beta(int_lead.fear))
  ...
  if(!(is.null(int_lead.fear_sw))){
    coefs_sw <- int_lead.fear_sw
