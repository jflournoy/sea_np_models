...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(ip_chavg.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(ip_chavg.fear))) {
    ...
    std_coefs <- coef(beta(ip_chavg.fear))
  ...
  if(!(is.null(ip_chavg.fear_sw))){
    coefs_sw <- ip_chavg.fear_sw
