...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(socanx.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(socanx.fear))) {
    coefs <- coef(summary(socanx.fear))
    ...
    std_coefs <- coef(beta(socanx.fear))
  ...
  if(!(is.null(socanx.fear_sw))){
    coefs_sw <- socanx.fear_sw
