...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(dep_lead.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(dep_lead.fear))) {
    ...
    std_coefs <- coef(beta(dep_lead.fear))
  ...
  if(!(is.null(dep_lead.fear_sw))){
    coefs_sw <- dep_lead.fear_sw
