...
processVoxel <- function(v) {
  ...
  model_formula <- GAD7_TOT_lead ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(gad_lead.happy <-
              nlme::lme(GAD7_TOT_lead ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  e_sandwich <- try(gad_lead.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(gad_lead.happy))) {
    ...
    std_coefs <- coef(beta(gad_lead.happy))
  ...
  if(!(is.null(gad_lead.happy_sw))){
    coefs_sw <- gad_lead.happy_sw
