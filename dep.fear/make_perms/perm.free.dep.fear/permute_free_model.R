...
processVoxel <- function(v) {
  ...
  model_formula <- y_star ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  #This implements the Freedman-Lane procedures (Freedman and Lane, 1983)
  
  # system.time({
  p <- try({
    dep.fear.p <-
      nlme::lme(PHQ9_TOT ~ 1 + TIMECENTER + GCEN_BRAIN,
                random = ~1 | idnum, data = model_data, 
                method = "REML", na.action=na.omit)
    epsilon_z <- resid(dep.fear.p)
    P_j.epsilon_z <- epsilon_z[this_perm]
    Zy <- predict(dep.fear.p, level = 1)
  ...
  # system.time({
  e <- try(dep.fear <-
             nlme::lme(y_star ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
                       random = ~1 | idnum, data = model_data, 
                       method = "REML", na.action=na.omit) )
  # }) #.043
  ...
  # system.time({
  e_sandwich <- try(dep.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  # }) #.047
  ...
  if(!(is.null(dep.fear_sw))){
    coefs_sw <- dep.fear_sw
