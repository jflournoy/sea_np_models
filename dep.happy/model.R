...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(dep.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(dep.happy))) {
    coefs <- coef(summary(dep.happy))
  ...
  if(!(is.null(dep.happy_sw))){
    coefs_sw <- dep.happy_sw
