...
processVoxel <- function(v) {
  ...
  model_formula <- BRAIN ~ 1 + TIMECENTER + GCEN_CHRONICAVG + WCEN_CHRONICAVG
  ...
  e <- try(chavg.happy <-
              nlme::lme(BRAIN ~ 1 + TIMECENTER + GCEN_CHRONICAVG + WCEN_CHRONICAVG,
  ...
  e_sandwich <- try(chavg.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(chavg.happy))) {
    coefs <- coef(summary(chavg.happy))
    ...
    std_coefs <- coef(beta(chavg.happy))
  ...
  if(!(is.null(chavg.happy_sw))){
    coefs_sw <- chavg.happy_sw
