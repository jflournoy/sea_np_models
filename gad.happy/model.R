...
processVoxel <- function(v) {
  ...
  model_formula <- GAD7_TOT ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(gad.happy <-
              nlme::lme(GAD7_TOT ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  e_sandwich <- try(gad.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(gad.happy))) {
    coefs <- coef(summary(gad.happy))
  ...
  if(!(is.null(gad.happy_sw))){
    coefs_sw <- gad.happy_sw
