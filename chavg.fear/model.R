...
processVoxel <- function(v) {
  ...
  e_sandwich <- try(chavg.fear_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(chavg.fear))) {
    coefs <- coef(summary(chavg.fear))
    ...
    std_coefs <- coef(beta(chavg.fear))
  ...
  if(!(is.null(chavg.fear_sw))){
    coefs_sw <- chavg.fear_sw
