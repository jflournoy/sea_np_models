...
processVoxel <- function(v) {
  ...
  model_formula <- PANASPOS ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(panaspos.happy <-
              nlme::lme(PANASPOS ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  e_sandwich <- try(panaspos.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(panaspos.happy))) {
    ...
    std_coefs <- coef(beta(panaspos.happy))
  ...
  if(!(is.null(panaspos.happy_sw))){
    coefs_sw <- panaspos.happy_sw
