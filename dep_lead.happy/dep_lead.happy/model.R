...
processVoxel <- function(v) {
  ...
  model_formula <- PHQ9_TOT_lead ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(dep_lead.happy <-
              nlme::lme(PHQ9_TOT_lead ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  e_sandwich <- try(dep_lead.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(dep_lead.happy))) {
    ...
    std_coefs <- coef(beta(dep_lead.happy))
  ...
  if(!(is.null(dep_lead.happy_sw))){
    coefs_sw <- dep_lead.happy_sw
