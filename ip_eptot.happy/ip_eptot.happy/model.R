...
processVoxel <- function(v) {
  ...
  model_data <- na.omit(data.frame(
    IP_EPISODICTOT = IP_EPISODICTOT, 
    TIMECENTER = TIMECENTER, 
    GCEN_BRAIN = GCEN_BRAIN, 
    WCEN_BRAIN = WCEN_BRAIN, 
    idnum = idnum))

  model_formula <- IP_EPISODICTOT ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(ip_eptot.happy <-
              nlme::lme(IP_EPISODICTOT ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  e_sandwich <- try(ip_eptot.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(ip_eptot.happy))) {
    ...
    std_coefs <- coef(beta(ip_eptot.happy))
  ...
  if(!(is.null(ip_eptot.happy_sw))){
    coefs_sw <- ip_eptot.happy_sw
