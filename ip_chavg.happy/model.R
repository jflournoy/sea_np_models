...
processVoxel <- function(v) {
  ...
  model_data <- na.omit(data.frame(
    IP_CHRONICAVG = IP_CHRONICAVG, 
    TIMECENTER = TIMECENTER, 
    GCEN_BRAIN = GCEN_BRAIN, 
    WCEN_BRAIN = WCEN_BRAIN, 
    idnum = idnum))

  model_formula <- IP_CHRONICAVG ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN
  ...
  e <- try(ip_chavg.happy <-
              nlme::lme(IP_CHRONICAVG ~ 1 + TIMECENTER + GCEN_BRAIN + WCEN_BRAIN,
  ...
  e_sandwich <- try(ip_chavg.happy_sw <- clubSandwich::coef_test(e, vcov = "CR2"))
  ...
  if (!(is.null(ip_chavg.happy))) {
    ...
    std_coefs <- coef(beta(ip_chavg.happy))
  ...
  if(!(is.null(ip_chavg.happy_sw))){
    coefs_sw <- ip_chavg.happy_sw
